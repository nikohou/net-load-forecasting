...
def drop_days_with_nans_advances(df, threshold):
    ...
    print(
        f"Dropped {len(days_to_drop)} days with more than {100* (1-threshold)}% of the rows had {threshold*100}% NaNs"
    )
...
def dropped_days_plotted(df, days_to_drop):
    ...
    df_days_to_drop = pd.DataFrame(
        index=days_to_drop, data=[1] * len(days_to_drop), columns=["drop"]
    )
...
def physical_profile(row, df_irr):
    ...
    mc = ModelChain(
        ...
        location,
        aoi_model="physical",
        spectral_model="no_loss",  # these are my model chain assumptions
    )
    mc.run_model(df_irr)
...
def prepare_metadata_per_scenario(
    df_netload: pd.DataFrame,
    df_meta: pd.DataFrame,
    df_irr: pd.DataFrame,
    meta_data_scenario: str,
):
    assert (
        meta_data_scenario in ["META-1", "META-2", "META-3", "META-4", "META-5"]
    ), "meta_data_scenario must be one of ['META-1', 'META-2', 'META-3', 'META-4', 'META-5']"
    ...
    if meta_data_scenario == "META-1":
        ...
        df_meta_["azimuth"] = df_meta_["azimuth"].apply(
            lambda x: rounder(x, 45)
        )  # modifying the azimuth angle based on estimation
        list_metas.append(df_meta_)
        ...
        estimated_ac_capacity = (
            abs(df_netload.min().values[0]) / df_meta.shape[0]
        )  # equally distributed capacity estimated from the netload data
        ...
        for i in range(10):
            ...
            df_meta_[["latitude", "longitude"]] = df_meta_[
                ["latitude", "longitude"]
            ].mean()
            estimated_ac_capacity = (
                abs(df_netload.min().values[0]) / df_meta.shape[0]
...
def pv_generation_forecasts(list_meta_per_scenario, scenarios: list, df_irr):
    ...
    for meta_scen in scenarios:
        ...
        for df_meta in list_meta:
            df_pv_forecast = (
                df_meta.apply(lambda x: physical_profile(x, df_irr), axis=1)
                .T.sum(axis=1)
                .to_frame("pv_forecast")
            )
            pv_forecast_per_scenario.append(df_pv_forecast)

        pv_forecast_per_scenario = (
            pd.concat(pv_forecast_per_scenario, axis=1)
            .mean(axis=1)
            .to_frame(f"pv_forecast_{meta_scen}")
        )
...
def build_config(config_dataset):
    ...
    config.horizon_in_hours = (
        24 + 36 if config.METER == "2" else 4
    )  # in hours, 24 for the data gap in METER-2 and 36 for the day-ahead forecast horizon
    config.timestep_encoding = (
        ["hour", "minute"] if config.temp_resolution == 1 else ["quarter"]
    )
    config.datetime_encoding = (
        ...
        if config.use_datetime_encoding
        else None
    )
...
def predict_testset(config, model, ts, ts_covs, pipeline):
    ...
    if config["METER"] == "2":
        historics = [
            historic[(config.timesteps_per_hour * 24) :] for historic in historics
        ]  # since in METER 2 we always have to wait 24 hours for the data

    gt_inverse_transformed = pipeline.inverse_transform(ts)

    historics_gt = [
        gt_inverse_transformed.slice_intersect(historic) for historic in historics
    ]
